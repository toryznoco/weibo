...
@interface TRComposeViewConreoller () <UITextViewDelegate, TRComposeToolBarDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate>

@property (nonatomic, weak) TRTextView *textView;
...
@implementation TRComposeViewConreoller
...
{
    TRComposePhotosView *photosView = [[TRComposePhotosView alloc] initWithFrame:CGRectMake(0, 70, self.view.width, self.view.height - 70)];
...
- (void)setUpToolBar
{
    ...
    TRComposeToolBar  *toolBar = [[TRComposeToolBar alloc] initWithFrame:CGRectMake(0, y, self.view.width, h)];
    _toolBar = toolBar;
...
- (void)setUpTextView
{
    TRTextView *textView = [[TRTextView alloc] initWithFrame:self.view.bounds];
    _textView = textView;
    //  设置占位符
...
- (void)sendPicture
{
    ...
    _rightItem.enabled = NO;
    //  我引用你，你引用我
    [TRComposeTool composeWithStatus:status image:image success:^{
        //  提示用户发送成功
...
- (void)sendTitle
{
    [TRComposeTool composeWithStatus:_textView.text success:^{
        ...
        [MBProgressHUD showSuccess:@"发送文字失败"];
        NSLog(@"%@", error);
    }];
}
